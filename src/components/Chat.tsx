...
const QUERY_TEMPLATES: { [key: string]: string } = {
  'Create NFT': `Please mint me an NFT into that collection 
...
const ExplorerLink = ({ type, address }: { type: 'address' | 'tx'; address: string }) => {
  const network = 'devnet'; // or get from your app's state
  const baseUrl = `https://explorer.solana.com/${type}/${address}`;
  const url = network === 'mainnet' ? baseUrl : `${baseUrl}?cluster=${network}`;
...
const formatMessage = (content: string) => {
  try {
    ...
    if (jsonData?.priceInUSDC && jsonData?.tokenId) {
      setSolPrice(jsonData.priceInUSDC);
    ...
    if (cleanText.toLowerCase().includes('wallet address')) {
      // Extract address using a more robust regex
      const address = cleanText.match(/[a-zA-Z0-9]{32,44}/)?.[0];
...
const AddressText = ({ text }: { text: string }) => {
  ...
  // Find and extract the complete transaction ID
  const txMatch = cleanedText.match(/Transaction:?\s*([\s1-9A-HJ-NP-Za-km-z]{88})/i);
  const transactionId = txMatch ? normalizeBase58(txMatch[1]) : null;

  if (transactionId) {
    // Remove any standalone occurrences of the transaction ID or its parts
    cleanedText = cleanedText
      .replace(new RegExp(transactionId, 'g'), '') // Remove full transaction ID
      .replace(/\s*Transaction:?\s*$/, '') // Remove trailing "Transaction:" if it's at the end
      .replace(/\s+/g, ' ') // Clean up extra whitespace
      .trim();
  }

  // Now handle the text with addresses
    ...
    <span className="whitespace-pre-wrap">
      {parts.map((part, i) => {
        ...
        if (part.match(addressRegex)) {
          // Don't show explorer link if we have a transaction ID
          const showExplorerLink = !transactionId;
          return (
            <span key={i} className="inline-flex items-center gap-1.5 bg-slate-800/50 px-2 py-0.5 rounded-md">
        ...
        // Check if this part contains the transaction text
        if (transactionId && part.toLowerCase().includes('transaction')) {
          const [before, after] = part.split(/transaction:?/i);
          return (
            <span key={i}>
              {before}
              Transaction:{' '}
              <span className="inline-flex items-center gap-1.5 bg-slate-800/50 px-2 py-0.5 rounded-md group">
                <span className="font-mono text-sm" title={transactionId}>
    ...
    try {
      ...
      if (data.status === 'error') {
        ...
        if (cleanContent.trim()) {
          const agentMessage = { role: 'agent', content: cleanContent };
            ...
            {messages.length === 0 ? (
              ...
              messages.map((message, index) => (
                ...
                >
                  ...
                  <div
                    className={`message-card max-w-[85%] ${
                      message.role === 'user'
